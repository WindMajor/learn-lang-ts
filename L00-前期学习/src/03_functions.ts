...
// ==========================================
// 示例 6：剩余参数（Rest Parameters）也叫散装参数
// 使用场景：接受任意数量的同类型参数
// ==========================================
...
function concatStrings(separator: string, ...parts: string[]): string {
  return parts.join(separator); // 注意！这里和python是反着的。TS是 ["a", "b", "c"].join("-")；python是 "-".join(["a", "b", "c"])
}
...
function padding(topBottom: number, leftRight: number): void;
function padding(top: number, right: number, bottom: number, left: number): void;
function padding(top: number, right?: number, bottom?: number, left?: number): void {
  // 1 个参数：四边相同；2 个：上下、左右；4 个：分别指定
  const finalRight = right ?? top;
  const finalBottom = bottom ?? top;
  const finalLeft = left ?? right ?? top;
  console.log(`padding: ${top} ${finalRight} ${finalBottom} ${finalLeft}`);
}
