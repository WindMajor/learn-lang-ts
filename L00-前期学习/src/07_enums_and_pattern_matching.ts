...
const a = Status.OK; // a: Status

// const enum → 类型是字面量
const enum ConstStatus {
  OK = 200,
}
const b = ConstStatus.OK; // b: 200

// 字面量联合 → 也是一样
type Code = 200 | 404;
const c: Code = 200; // c: Code (只能等于 200 | 404)

// 一句话总结：const enum 比普通 enum 更"透明"：编译后消失，类型也退化为字面量值，和直接写数字没区别。它几乎就是字面量联合类型的另一种写法。
// 然后字面量联合实际上更优：错误信息友好（告诉你哪个类型不匹配），类型名不会蒸发。
// const enum 唯一的优势只剩 Status.OK 这种带命名空间的书写便利，但这个代价是丢了类型名。
// 这也是社区更推字面量联合的原因。
...
// HttpMethod 的类型：{ readonly GET: "GET"; readonly POST: "POST"; readonly PUT: "PUT"; readonly DELETE: "DELETE"; }
const HttpMethod = {
  GET: 'GET',
  POST: 'POST',
  PUT: 'PUT',
...
// typeof → 取对象的类型，不是字符串 { readonly GET: "GET"; readonly POST: "POST"; readonly PUT: "PUT"; readonly DELETE: "DELETE" }
// keyof  → 取所有键的联合 "GET" | "POST" | "PUT" | "DELETE"
// T[K]   → 取键对应的值类型 (typeof HttpMethod)["GET" | "POST" | "PUT" | "DELETE"] 得到 "GET" | "POST" | "PUT" | "DELETE"
// 三者套用 = 从 as const 对象提取值联合
type HttpMethod = (typeof HttpMethod)[keyof typeof HttpMethod]; // 注意这是类型签名语句里，typeof 和 运行时的 typeof 不是同一个
// 上面这一句很复杂，实际等价于 type HttpMethod = "GET" | "POST" | "PUT" | "DELETE";
// 好处是改对象自动同步类型，不用手动维护两处

// 上面有1个HttpMethod常量，1个HttpMethod类型。TS 里值和类型可以同名共存！
// TS 有值命名空间和类型命名空间，不冲突。值空间：运行时存在；类型空间：编译后消失。
