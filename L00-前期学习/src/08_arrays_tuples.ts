...
const mixed2: Array<string | number> = ['a', 1, 'b', 2];
...
const copy = [...original]; // 新建了一个数组，元素按引用复制
copy[0] = 'x'; // 改了 copy，不影响 original
console.log(original); // ["a", "b", "c"]（不变）  "a" ← 没事，因为字符串是原始值

// 什么时候浅拷贝会露馅？—— 数组里装对象时（数组存的是引用（指针））
// ❌ 浅拷贝暴露问题：里层元素是引用，共享同一份
const arr3 = [{ name: 'Alice' }, { name: 'Bob' }];
const copy2 = [...arr3];

copy2[0]!.name = 'Charlie';
console.log(arr3[0]!.name); // "Charlie" ← original 也被改了！
// 外层数组是独立的，但里面的对象是同一个引用
...
// 上面的 item is string 就是类型谓词，自定义类型守卫的返回类型，它替代了 boolean 的位置，但运行时返回值依然是 true/false，它就是 boolean 的"升级款"
// 如果没有类型谓词，filter 后还是 (string | number)[]
// 有类型谓词 filter 后是 string[]
// item is string 是给 filter 的"承诺书"——"我帮你验证过了，这里面的东西确实是 string，放心收窄"。
console.log(onlyStrings); // ["a", "b", "c"]

// reduce 需要显式注解累加器类型
const sum = scores.reduce((total: number, curr) => total + curr, 0);
console.log(sum);
