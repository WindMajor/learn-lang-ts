...
const privateCache = new WeakMap<object, CacheData>(); // 弱引用Map的键必须是对象object
...
// WeakMap 不可遍历 — 没有 .size / .keys() / .values() / .entries() / for...of
// 因为键是弱引用，随时可能被 GC 回收，所以无法暴露完整的键列表
// privateCache.size;       // ❌ 不存在
// for (const [k, v] of privateCache) {} // ❌ 不可迭代
...
// Record 与索引签名的区别：
// - Record<K, T>：键的范围被 K 精确限定，访问不存在的键会报错
// - { [key: string]: T }：接受任意字符串键，但失去了精确的键约束

// Record 示例：键必须是 'admin' | 'editor' | 'viewer'
const r1 = rolePermissions.admin; // ✅ { read: true, write: true }
// rolePermissions.unknown;               // ❌ 属性 "unknown" 不存在

// 索引签名示例：任意字符串键都可以，但没有键名的自动补全
const idxConfig: { [key: string]: Permissions } = rolePermissions;
const r2 = idxConfig['admin']; // ✅ 可以，但没有类型安全的键名检查
const r3 = idxConfig['anything']; // ✅ 也可以（但值是 Permissions | undefined）
// 错误写法：idxConfig.anything            // ❌ 不能通过点号访问非联合键
// 索引签名只能通过方括号访问，点号访问只对已知键生效
