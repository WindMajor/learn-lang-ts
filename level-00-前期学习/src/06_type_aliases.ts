...
type Callback = (data: string) => void; // 本质是一个函数，本身可调用
...
// ==========================================
// TS 故意不允许直接 'hello' as number，因为 string 和 number 没有交集
// unknown 是所有类型的父类型，任何类型都能先转 unknown，再从 unknown 转任意类型
const weirdValue = 'hello' as unknown as number;
// 先将 string 断为 unknown，再断为 number
// 这完全破坏了类型安全，应极力避免！
console.log(weirdValue);
// as any 比双重断言更危险，全部后续操作都无检查，难以定位bug来源，直接关闭灭火系统。
// 双重断言好歹还会去到一个特定类型，还有检查
...
// 社区规范是优先使用interface，只有在interface不支持的场景使用type
...
// ✅ Circle 和 Rectangle 都是对象形状 → 用 interface
interface Circle {
  kind: 'circle';
  radius: number;
}
...
// string & number; 没有值同时是 string 和 number，只能是never
